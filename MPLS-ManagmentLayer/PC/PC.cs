...
namespace ControlPlane
{
    ...
    class PC
    {
        ...
        public PC(string configurationFilePath, CPCC pointerCPCC)
        {

            InitialiseVariables(configurationFilePath);
            InitializeSocket();
        _moduleCPCC = pointerCPCC;
        ...
        private void ReceiveOutsidePacket(byte[] receivedPacket)
        {
            ...
            {
                case "CPCC":
                   receiveMessage = new Delegate_ReceiveOutsideMessage(_moduleCPCC.ReceiveMessageFromPC);
                    break;
                default:
                    //SignallingNode.MakeSignallingLog("PC", "ERROR - Destination module unknown.");
                    break;
            }
